...
exports.normalizeListingPayload = exports.normalizeAsset = exports.normalizePricing = void 0;
...
exports.normalizePricing = normalizePricing;
const normalizeAsset = (input) => {
    if (!input || typeof input !== "object")
        return undefined;
    const a = { ...input };
    if (!a.type)
...
const normalizeListingPayload = (payload) => {
    ...
    const listingPricing = (0, exports.normalizePricing)(p);
    if (listingPricing)
    ...
    if (Array.isArray(p.floorPlans)) {
        p.floorPlans = p.floorPlans.map((fp) => {
            const plan = { ...(fp || {}) };
            const planPricing = (0, exports.normalizePricing)(plan);
            if (planPricing)
                plan.pricing = planPricing;
            delete plan.price;
            delete plan.currency;
            if (plan.image) {
                plan.image = (0, exports.normalizeAsset)(plan.image);
            }
            return plan;
        });
    }
...
exports.normalizeListingPayload = normalizeListingPayload;
